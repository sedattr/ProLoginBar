...
public class BossBar {
    ...
    public void create(Player player, Integer time) {
        ...
        org.bukkit.boss.BossBar bar = Bukkit.createBossBar(Utils.colorize(title.replace("%time%", String.valueOf(time))), BarColor.valueOf(color), BarStyle.valueOf(style));
    ...
    public void change(Player player, Integer time, Integer kickTime) {
        ...
        if (this.section.getBoolean("progress")) {
            ...
            bar.setProgress(Math.max(bar.getProgress() - removeCount, 0.0));
        }
        ...
        if (title == null || title.equals(""))
            return;
        bar.setTitle(Utils.colorize(title.replace("%time%", String.valueOf(time))));
    }
